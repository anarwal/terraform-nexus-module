resource "aws_security_group" "internal_ssh" {
  name_prefix = format("%s-%s-internal-ssh-", module.nexus_label.name, module.nexus_label.stage)
description = "Allows ssh from bastion"
vpc_id      = var.vpc_id
tags        = { "Name" = format("%s-internal-ssh",module.nexus_label.name),"Environment" = module.nexus_label.stage }
...
name_prefix = format("%s-%s-external-ssh-", module.nexus_label.name, module.nexus_label.stage)
description = "Allows ssh from the world"
vpc_id      = var.vpc_id
tags        = { "Name" = format("%s-external-ssh",module.nexus_label.name), "Environment" = module.nexus_label.stage }
